;vortex tracker ii v1_0 pt3 player for zx spectrum
;(c)2004,2007 s_v_bulba <vorobey@mail_khstu_ru>
;http://bulba_untergrund_net (http://bulba_at_kz)
	device zxspectrum48
;release number
release equ "7"
...
;--------
;-can be compiled at any address (i_e_ no need rounding org
; address)_
;-variables (vars) can be located at any address (not only after
;code block)_
;-init subroutine detects module version and rightly generates
; both note and volume tables outside of code block (in vars)_
;-two portamento (spc_ command 3xxx) algorithms (depending of
; module version)_
;-new 1_xx and 2_xx special command behaviour (only for pt v3_7
; and higher)_
;-any tempo value are accepted (including tempo=1 and tempo=2)_
;-fully compatible with ay_emul pt3 player codes_
;-see also notes at the end of this source code_

;limitations
;-----------
;-can run in ram only (self-modified code is used)_

;warning!!! play subroutine can crash if no module are loaded
;into ram or init subroutine was not called before_
...
tp_2	ld a,h
	...
	ld (chanc+chp_ornptr),hl ;3_xx to 3_6x and vtii
...
;(xx1b - 3_xx__3_4r, xx0b - 3_4x__3_6x__vtii1_0)
...
;a - versionforvolumetable (0__4 - 3_xx__3_4x;
			   ;5__ - 3_5x__3_6x__vtii1_0)
...
version equ $+1
	ld a,0x3e
	cp 6
	jr c,oldprtm ;old 3xxx for pt v3_5-
...
no_enac	ld hl,addtoen
	add a,(hl) ;bug in pt3 - need word here_
...
t_pack	db 0x06ec*2/256,0xd8 ;0x06ec*2
	db 0x0755-0x06ec
	db 0x07c5-0x0755
	db 0x083b-0x07c5
	db 0x08b8-0x083b
...
;release 0 steps:
;11_sep_2004 - note tables creator
;12_sep_2004 - volume tables creator; init subroutine
;13_sep_2004 - play counters, position counters
;14_sep_2004 - patterns decoder subroutine
;15_sep_2004 - resting (no code)
;16_sep_2004 - chregs subroutine; global debugging; 1st stable
;version was born
;17_sep_2004 - debugging and optimization_ first release!
;release 1 steps:
;20_sep_2004 - local vars moved to code (selfmodified code
;smaller and faster)
;22_sep_2004 - added mute sound entry at start+8; position
;pointer moved to start+11; added setup and status byte at
;start+10 noloop mode and loop passed flags added
;release 2 steps:
;28_sep_2004 - optimization: code around chregs's volume and
...
;release 3 steps:
;2_oct_2004 - optimization in init and pd_loop (thanks to ivan
;roshin)
;4_oct_2004 - load delay from (hl) in init (2 bytes shorter)
;5_oct_2004 - optimization in pd_loop (->pd_lp2)
;7_oct_2004 - swaping some commands for better packing
;release 4 steps:
;9_oct_2004 - optimization around ld hl,spccoms (thanks to ivan
;roshin); in ptdecod swapped bc and de to optimize c_portm;
;removed sam and orn len and loop channel vars; chregs totally
;optimized
;release 5 steps:
;11_oct_2004 - pd_orsm and c_portm optimized; ivan roshin's
;volume tables creator algorithm (51 bytes shorter than mine)
;12_oct_2004 - ivan roshin's note tables creator algorithm (74
;bytes shorter than mine)
;release 6 steps:
;14_oct_2004 - loop and next position calculations moved to init
;15_oct_2004 - adinpt moved to code
;19_oct_2004 - env_del moved to code
;20_oct_2004 - version push and pop (1 byte shorter, thanks to
;ivan roshin)
;22_oct_2004 - env_en moved from flags' bit to byte (shorter and
;faster code)
;25_oct_2004 - setenv optimized
;29_oct_2004 - optimized around addtoen (sbc a,a, thanks to ivan
;roshin)
;3_nov_2004 - note tables data was compressed; with depacker it
;is 9 bytes shorter than uncompressed (thanks to ivan roshin)
;4_nov_2004 - default sample and ornament both are fixed now
;and placed into code block (6 bytes shorter)
;7_nov_2004 - ld a,(ns_base):ld l,a changed to ld hl,(ns_base)
;(thanks to dima bystrov)
;9_nov_2004 - ns_base and addtons are merged to ns_base_addtons;
;ld a,255 changed to dec a (at start of play); added rout_a0
;12_nov_2004 - ntskcn&volume are merged (8 bytes smaller init);
;ld bc,t1_ changed to push de___pop bc in note table creator
;19_dec_2004 - nt_data reorganized (6 bytes shorter, thanks to
;ivan roshin); c_portm and c_gliss are merged via set_stp (48
;tacts slower, but 8 bytes smaller, thanks to ivan roshin)
;release 7 steps:
;29_apr_2007 - sjasm adaptation; new 1_xx and 2_xx
;interpretation for pt 3_7+_
																
																

;tests in immation tester v1_0 by andy man/pos (thanks to
...
;size:
;code block 0x651 bytes
;variables 0x21d bytes (can be stripped)
;size in ram 0x651+0x21d=0x86e (2158) bytes

;notes:
;pro tracker 3_4r can not be detected by header, so pt3_4r tone
;tables really used only for modules of 3_3 and older versions_
